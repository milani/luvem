...
local core = require('luvem.core')
local Stream = require('luvem.stream.stream_core').Stream
...
function WritableState:initialize(options, stream)
  ...
  if core.instanceof(stream, require('luvem.stream.stream_duplex').Duplex) then
